# script to plot linearity or the lack of it for the impact switches in FRIDA
# Read all the variables from single excel file and select specific variables using dplyr::filter

# Revised version of the original script, to make use of the same ensembles for all the plots for consistency
# Revised for producing plot for EGU talk. We don't need all the variables, also need to panelise according to modules
...
remove(list = ls())
filepath.calib      <- ("G:/My Drive/R/r-scripts/worldTrans/WorldTransFrida-Uncertainty/workOutput")
...
calib.df         <- read.csv2(paste(filepath.calib,"/","Calibration Data.csv",sep=""), sep=",")[,-c(46:52)]
names(calib.df)  <- substring(names(calib.df),2,5)   # keep last 4 letters in the column names (YYYY format)

energy.demand.calibration          <- filter(calib.df, if_any(everything(),~str_detect(.x,"energy demand.demand for energy")))
food.demand.calibration            <- filter(calib.df, if_any(everything(),~str_detect(.x,"Food Demand.direct food demand per person per day")))
concrete.use.calibration           <- filter(calib.df, if_any(everything(),~str_detect(.x,"Concrete.total yearly concrete use")))
births.calibration                  <- filter(calib.df, if_any(everything(),~str_detect(.x,"Demographics.Births")))
mortality.calibration              <- filter(calib.df, if_any(everything(),~str_detect(.x,"Demographics.Total Deaths")))
population.calibration              <- filter(calib.df, if_any(everything(),~str_detect(.x,"Demographics.Population")))
#water.use.calibration              <- filter(calib.df, if_any(everything(),~str_detect(.x,"Freshwater.total water withdrawal")))
gdp.per.person.calibration         <- filter(calib.df, if_any(everything(),~str_detect(.x,"real GDP per person")))
govt.consumption.calibration       <- filter(calib.df, if_any(everything(),~str_detect(.x,"Government.Government Consumption")))
sta.calibration                    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Surface Temperature Anomaly")))
slr.calibration                    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Total global sea level anomaly")))
total.co2.emissions.calibration    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Emissions.Total CO2 Emissions")))
total.ch4.emissions.calibration    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Emissions.Total CH4 Emissions")))
total.n2o.emissions.calibration    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Emissions.Total N2O Emissions")))
total.so2.emissions.calibration    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Emissions.Total SO2 Emissions")))
total.nox.emissions.calibration    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Emissions.Total NOx Emissions")))
total.vox.emissions.calibration    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Emissions.VOC Emissions")))
total.co.emissions.calibration     <- filter(calib.df, if_any(everything(),~str_detect(.x,"Emissions.CO Emissions")))
total.hfc.emissions.calibration    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Emissions.HFC134a eq Emissions")))
...
#total.ghg.emissions.calibration  <- apply(array(as.numeric(unlist(total.ghg.emissions.calibration)),dim=c(8,43)),2,function(x) sum(x))
...
#water.use.calibration              <- data.frame(as.factor(seq(1980,2150)),abind(as.numeric(t(water.use.calibration[,-1])),rep(NA,len.frida.data-44)))
...
#total.rad.forcing.calibration      <- data.frame(as.factor(seq(1980,2150)),rep(NA,len.frida.data))
...
plot.fun <- function(model.data,calibration.data,multiplier,y.label,title.label){
  ...
  # plot only EMB
  #scale_color_manual(values=c("black","red"),
  #                   breaks=c("Endogenous Model Behavior"))+
  #scale_fill_discrete(type=c("black","red"),
  #                  breaks=c("Endogenous Model Behavior"))+
  theme(axis.text.x = element_text(family = "sans",size=8,angle=45, vjust=0.3, color="black"),
        axis.text.y = element_text(family = "sans",size=10, vjust=0.3, color="black"),
...
print(consumption.plot)

finance.plot <- ggarrange(gdp.plot, productivity.plot, consumption.plot,
                       nrow=1, ncol=3,
...
print(finance.plot)
ggsave(
  filename = "Figure_finance_plot.png",
  plot = finance.plot,
  width = 11,
  height = 4)
...
demography.plot <- ggarrange(population.plot, mortality.plot, births.plot,
                          nrow=1, ncol=3,
                          heights= c(1,1,1),widths= c(1,1,1),
                          align= "v", common.legend = T, legend="bottom")
...
ggsave(
  filename = "Figure_demography_plot.png",
  plot = demography.plot,
  width = 11,
  height = 4)
...
consumption.plot <- ggarrange(energy.demand.plot, food.demand.plot, crop.yield.plot,
                              nrow=1, ncol=3,
                              heights= c(1,1,1),widths= c(1,1,1),
                              align= "v", common.legend = T, legend="bottom")

ggsave(
  filename = "Figure_consumption_plot.png",
  plot = consumption.plot,
  width = 11,
  height = 4)
...
climate.plot <- ggarrange(emissions.plot, sta.plot, slr.plot,
                              nrow=1, ncol=3,
                              heights= c(1,1,1),widths= c(1,1,1),
                              align= "v", common.legend = T, legend="bottom")

ggsave(
  filename = "Figure_climate_plot.png",
  plot = climate.plot,
  width = 11,
  height = 4)
...
draw.plot <- ggarrange(gdp.plot, productivity.plot, consumption.plot,
...
ggsave(
  filename = "Figure_1_EMBvsCounterfactual_v2.png",
  plot = draw.plot,
  width = 13,
  height = 12)
